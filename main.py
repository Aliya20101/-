...
class ExpenseTrackerApp:
    def __init__(self, root):
        ...
        self.add_button = tk.Button(
            root, text="Добавить расход", command=self.add_expense
        )
        ...
        self.sum_button = tk.Button(
            root, text="Сумма за период", command=self.calculate_sum
        )
        self.sum_button.grid(row=7, columnspan=2, pady=5)

        # Фильтр по категории и дате
        tk.Label(root, text="Фильтр по категории:").grid(
            row=8, column=0, sticky="e", padx=5
        )
        ...
        self.filter_button = tk.Button(
            root, text="Фильтровать", command=self.filter_expenses
        )
